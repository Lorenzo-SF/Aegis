defmodule Aegis.Terminal.Kitty do
  ...
  def create_window(window_id, opts) do
    ...
    exec_raw!(cmd, capture_output: false)
  end

  def create_tab(tab_id, command, opts) do
    shell = Keyword.get(opts, :shell, @default_shell)
    ...
    with {:ok, _} <- exec_raw!(base_cmd, capture_output: false) do
      if command do
        Process.sleep(100)
        send_command(command, tab_id: kitty_tab_id)
      else
        {:ok, ""}
      end
    end
  ...
  def create_pane(pane_id, orientation, command, opts) do
    shell = Keyword.get(opts, :shell, @default_shell)
    ...
    with {:ok, _} <- exec_raw!(base_cmd, capture_output: false) do
      if command do
        Process.sleep(100)
        send_command(command, pane_id: kitty_pane_id)
      else
        {:ok, ""}
      end
    end
  ...
  def send_command(command, opts) do
    case extract_kitty_target(opts) do
      nil ->
        exec_raw!(["kitty", "@", "send-text", "\"#{command}\"\r"], capture_output: false)

      target ->
        exec_raw!(["kitty", "@", "send-text", "--match", target, "\"#{command}\"\r"], capture_output: false)
    end
  end
  ...
  def close_element(opts) do
    case extract_kitty_target(opts) do
      ...
      target ->
        with {:ok, output} <- exec_raw!(["kitty", "@", "ls", "--format=json"], capture_output: true),
             {:ok, windows_data} <- Jason.decode(output),
             target_window <- find_target_window(windows_data, target),
             true <- not is_nil(target_window) do
          exec_raw!(["kitty", "@", "close-window", "--match", "id:#{target_window["id"]}"], capture_output: false)
        else
  ...
  def apply_styles(opts) do
    ...
    if tab_id && colors do
      ...
      target = "title:#{tab_id}"
      exec_raw!(["kitty", "@", "set-colors", "--match", target, "foreground=#{fg}", "background=#{bg}"], capture_output: false)
    else
  ...
  def apply_layout(opts) do
    ...
    exec_raw!(["kitty", "@", "goto-layout", layout], capture_output: false)
  end
  ...
  def navigate_to(opts) do
    cond do
      Keyword.has_key?(opts, :tab_id) ->
        ...
        exec_raw!(["kitty", "@", "focus-tab", "--match", "title:#{kitty_tab_id}"], capture_output: false)
      ...
      Keyword.has_key?(opts, :window_id) ->
        ...
        exec_raw!(["kitty", "@", "focus-window", "--match", "title:#{kitty_window_id}"], capture_output: false)
  ...
  def list_elements do
    exec_raw!(["kitty", "@", "ls"], capture_output: true)
  end
  ...
  def resize(height, width) do
    exec_raw!(["kitty", "@", "resize-os-window", "--self", "--width", to_string(width), "--height", to_string(height)], capture_output: false)
    Process.sleep(100)
    ...
    exec_raw!(["osascript", "-e", position_script], capture_output: false)
  end
  ...
  defp extract_kitty_target(opts) do
    cond do
      Keyword.has_key?(opts, :pane_id) ->
        "title:KITTY_PANE_#{String.upcase(opts[:pane_id])}"

      Keyword.has_key?(opts, :tab_id) ->
        "title:KITTY_TAB_#{String.upcase(opts[:tab_id])}"

      Keyword.has_key?(opts, :window_id) ->
        "title:KITTY_WINDOW_#{String.upcase(opts[:window_id])}"
  ...
  defp assign_services_to_panes_corrected(services) do
    ...
    |> Enum.each(fn {service, index} ->
      if index < length(position_titles) do
        ...
        if command != "" do
          exec_raw!(["kitty", "@", "send-text", "--match", "title:#{title}", "\"#{command}\"\r"], capture_output: false)
          Process.sleep(150)
        end
      end
    end)
